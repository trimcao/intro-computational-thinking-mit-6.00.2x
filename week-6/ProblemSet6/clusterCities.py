...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        for point1 in self.members():
            for point2 in other.members():
                currentDist = point1.distance(point2)
    ...
    def averageLinkageDist(self, other):
        ...
        for point1 in self.members():
            for point2 in other.members():
                numPairs += 1
                sumDist += point1.distance(point2)
        return (sumDist / numPairs)
            
        #dimensions = self.points[0].dimensionality() 
        #numPoints1 = 0
        #aveAttrs1 = [0 for idx in range(dimensions)]
        #for point1 in self.members():
        #    numPoints1 += 1
        #    attrs = point1.getAttrs()
        #    for idx in range(dimensions):
        #        aveAttrs1[idx] += attrs[idx]
        ## find the average
        #for idx in range(dimensions):
        #    aveAttrs1[idx] = aveAttrs1[idx] / float(numPoints1)
        #avePoint1 = Point('point1', aveAttrs1)
        ## find the average point 2
        #numPoints2 = 0
        #aveAttrs2 = [0 for idx in range(dimensions)]
        #for point2 in other.members():
        #    numPoints2 += 1
        #    attrs = point2.getAttrs()
        #    for idx in range(dimensions):
        #        aveAttrs2[idx] += attrs[idx]
        ## find the average
        #for idx in range(dimensions):
        #    aveAttrs2[idx] = aveAttrs2[idx] / float(numPoints2)
        #avePoint2 = Point('point2', aveAttrs2)
        #return avePoint1.distance(avePoint2)
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        for point in c2.members():
            newPoints.append(point)
        newCluster = Cluster(newPoints, list)
        self.add(newCluster)
    ...
    def findClosest(self, linkage):
        ...
        cluster1 = None
        cluster2 = None
        for i in range(self.numClusters() - 1):
            for j in range(i + 1, self.numClusters()):
                currentValue = linkage(self.members[i], self.members[j])
    ...
    def mergeOne(self, linkage):
        ...
        # TO DO
        closestClus = self.findClosest(linkage)
        c1 = closestClus[0]
        c2 = closestClus[1]
